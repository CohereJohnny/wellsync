-- Migration to DROP and RECREATE search_faults RPC (correct timestamp type)

-- Drop the previous version
DROP FUNCTION IF EXISTS search_faults(vector, double precision, integer);

-- Create the function with corrected RETURNS TABLE definition for timestamp
...
RETURNS TABLE (
  ...
  "timestamp" TIMESTAMP, -- Corrected type to TIMESTAMP WITHOUT time zone
  fault_type TEXT,
  similarity float
)
LANGUAGE plpgsql
