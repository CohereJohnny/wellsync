-- Migration to DROP and RECREATE search_faults RPC (correct part_id type)

-- Drop the previous version
DROP FUNCTION IF EXISTS search_faults(vector, double precision, integer);

-- Create the function with corrected RETURNS TABLE definition for part_id
...
RETURNS TABLE (
  fault_id UUID,
  well_id UUID,
  part_id character varying(10), -- Corrected type to varchar(10)
